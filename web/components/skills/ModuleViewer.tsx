...
export function ModuleViewer({ module, skillSlug, isCompleted, nextModule, isLastModule }: Props) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      setDone(true);
      if (nextModule) {
        router.push(`/skills/${skillSlug}?module=${nextModule.order}`);
        router.refresh();
      } else {
        router.refresh();
      }
  ...
  return (
    <article className="bg-white rounded-xl border p-8">
      <div className="mb-6">
        <p className="text-sm text-gray-400 uppercase tracking-wide mb-1">Module {module.order}</p>
      ...
      <div className="prose prose-gray max-w-none">
        <ReactMarkdown
          components={{
            ...
            blockquote: ({ children }) => (
              <blockquote className="border-l-4 border-blue-400 pl-4 py-1 my-4 text-gray-600 italic">{children}</blockquote>
            ),
            table: ({ children }) => (
            ...
            code: ({ children, className }) => {
              ...
              if (isInline) return <code className="bg-gray-100 px-1.5 py-0.5 rounded text-sm font-mono text-gray-800">{children}</code>;
              return <code className="block bg-gray-900 text-gray-100 p-4 rounded-lg text-sm font-mono overflow-x-auto my-4">{children}</code>;
            },
        ...
        <div className="flex items-center gap-3">
          {!done && (
            <Button onClick={handleComplete} disabled={marking}>
              {marking ? "Marking..." : isLastModule ? "Complete Skill ✓" : "Mark Complete & Continue →"}
            </Button>
          )}
          {done && nextModule && (
            <Button
              variant="outline"
              onClick={() => router.push(`/skills/${skillSlug}?module=${nextModule.order}`)}
            >
              Next Module →
            </Button>
          )}
